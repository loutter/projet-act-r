...
(defun show-learning (n)
  (let ((data nil))
    ...
    (let ((stats (mapcar (lambda (experience) (mapcar (lambda (x) (/ x n)) (butlast experience))) data) ))
    (let ((stats (mapcar (lambda (experience) (mapcar (lambda (x) (/ x n)) (last experience))) data) ))
      (draw-graph stats)
    )
  )
)
